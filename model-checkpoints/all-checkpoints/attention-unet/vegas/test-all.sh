...
for model_checkpoint in "$MODEL_DIR"/*.pth; do
  ...
  echo "Processing $model_checkpoint for Vegas..."
  python -u test-model-custom.py --test-imgs "$TEST_IMGS_DIR" --test-masks "$TEST_MASKS_DIR" --load "$model_checkpoint" --dataset Vegas 2>&1 | tee "$LOG_DIR/${model_name}_Vegas.txt"
  echo "Output for Vegas saved to ${model_name}_Vegas.txt"
done

echo "All models processed for Vegas."
